...
\section{N=8 feasibility check}

We measured per-$\Phi$ wall time at $N=8$ ($G=9$, 729 cells) using the same
numba pipeline. Theoretical scaling is $(9/7)^3 \cdot (9/7)^2 \approx 4.4\times$
the $N=6$ cost.
...
Measured matches theory. With $\sim 150$ symmetric DOFs at $N=8$, a Newton outer
iteration would be $\sim 150 \cdot 1.18 \approx 180$ s ($\sim 3$ min), and a
full 10-iter Newton would be $\sim 30$ min on 1 CPU. \textbf{$N=8$ is
practical} with the existing numba pipeline; $N=10$ at $\sim 16\times$ over
$N=6$ would be $\sim 1.5$ h per Newton iter, borderline but doable for a
single $\gamma$-step gold test.
