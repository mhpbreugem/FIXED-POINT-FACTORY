"""Gamma sweep: linear-CDF STRICT h=0 (POU + line integral) tabulated mu."""
...
from lin_cdf_strict import phi_lin_strict, make_cdf_uniform_grid, make_p_grid
...
for nq in [12, 14, 16, 18, 20, 24]:
    _ = phi_lin_strict(sg(0.5*T_full), u_grid, NQ=nq, G_p=121)

OUT = '/tmp/cheby_h0/lin_strict_gamma.json'
FPS = '/tmp/cheby_h0/fps_lin_strict'
...
for gamma in GAMMAS:
    ...
    for NQ in NQs:
        F_func = lambda x_flat, NQ=NQ, gamma=gamma: \
            (phi_lin_strict(x_flat.reshape(G,G,G), u_grid, NQ=NQ,
    ...
    np.save(f'{FPS}/P_FP_lin_strict_gamma{gamma}.npy', P_fp)
