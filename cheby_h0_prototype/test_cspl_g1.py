"""Quick test: C^2 cubic spline at gamma=1, NQ sweep, can we hit machine eps?"""
...
for nq in [12, 14, 16, 18, 20, 24]:
    _ = phi_lin_cspl(sg(0.5*T_full), u_grid, NQ=nq, G_p=121, gamma=1.0)

print(f'{"NQ":>4} {"And":>14} {"+NK":>14} {"slope":>9} {"def_1to1":>10} {"t":>6}',
      flush=True)
for NQ in [12, 14, 16, 18, 20, 24]:
    F_func = lambda x_flat, NQ=NQ: \
        (phi_lin_cspl(x_flat.reshape(G,G,G), u_grid, NQ=NQ, G_p=121,
                          tau=1.0, gamma=1.0)
          - x_flat.reshape(G,G,G)).ravel()
    x0 = sg(0.5*T_full).ravel()
    t0 = time.time()
    ...
    print(f'{NQ:>4d} {min(Fs):>14.3e} {f_nk:>14.3e} {s:>9.4f} {def_1to1:>10.4f} '
