"""V5 KERNEL CO-AREA gamma-sweep at G_inner=10 -- test CRRA -> CARA (FR) as gamma -> infty.

Each gamma in {0.1, 0.3, 1, 3, 10, 30, 100}: Picard with V5 kernel co-area op
from no-learning IC, 20 iters, G_FULL=12 (G_inner=10) for tractability.

Hellwig prediction: CRRA gap ~ 1/gamma. As gamma grows, the FP should approach
the CARA = FR equilibrium: 1-R^2 -> 0, slope -> 1, d_FR_w -> 0.
"""
...
LOG = os.path.join(HERE, 'flint_v5_gamma_sweep.log')
open(LOG, 'w').close()
def lg(m):
...
for gamma_val in GAMMAS:
    ...
    results.append(rec)
    np.save(os.path.join(HERE, f'flint_v5_gamma{gamma_val:g}_G{G_INNER}_P.npy'), inner)
    ...
    json.dump({'tau':TAU_F, 'G_FULL':G_FULL, 'dps':DPS, 'h':float(h_kernel), 'results':results,
               'note':'V5 kernel co-area; test CRRA -> CARA(FR) as gamma -> infty'},
              open(os.path.join(HERE, 'flint_v5_gamma_sweep.json'), 'w'), indent=2)
