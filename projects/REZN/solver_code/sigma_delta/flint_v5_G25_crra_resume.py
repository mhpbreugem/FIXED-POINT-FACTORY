"""Resume V5 CRRA gamma=0.1 G=25 from the saved iter-4 P. 10 more iters."""
...
G_FULL = 27; INNER_LO, INNER_HI = 1, G_FULL - 1; G_INNER = INNER_HI - INNER_LO
...
LOG = os.path.join(HERE, 'flint_v5_G25_crra_resume.log')
open(LOG, 'w').close()
def lg(m):
...
tau = mp(TAU_F); gamma_mp = mp(GAMMA_F); W = mp(W_F)
...
# Load iter-4 P (CRRA gamma=0.1)
P_iter4 = np.load(os.path.join(HERE, 'flint_v5_G25_edge_crra_g0p1_NL_IC_P.npy'))
lg(f'Resume from iter-4 P: slope={w_R2(P_iter4)[1]:.5f}  d_FR_w={d_FR_w(P_iter4):.3e}  1-R²={w_R2(P_iter4)[0]:.3e}')

P_full_np = np.zeros((G_FULL,)*3); P_full_np[INNER_LO:INNER_HI, INNER_LO:INNER_HI, INNER_LO:INNER_HI] = P_iter4
P = [[[mp(float(P_full_np[i,j,k])) for k in range(G_FULL)] for j in range(G_FULL)] for i in range(G_FULL)]
P = set_boundary(P)

omega = mp('0.7'); res_prev = mp('8.95e-2')  # approx last ferr
iter_data = {'res':[], 'omega':[], 'd_FR_w':[], 'slope':[], '1mR2':[], 'sec':[]}
t0 = time.time()
for it in range(5, 5 + MAX_ITER):
    t_step = time.time()
    ...
    lg(f'  it {it:3d}  ω={float(omega):.3f}  ferr={res:.3e}  d_FR_w={d_w:.3e}  slope={slope:.5f}  1-R²={omr2:.3e}  ({sec:.0f}s)')
    np.save(os.path.join(HERE, 'flint_v5_G25_edge_crra_g0p1_NL_IC_P.npy'), inner)
    json.dump({'tau':TAU_F, 'gamma':GAMMA_F, 'G_FULL':G_FULL, 'G_inner':G_INNER, 'dps':DPS,
               'h':float(h_kernel), 'iter_data':iter_data, 'resumed_from':4},
              open(os.path.join(HERE, 'flint_v5_G25_crra_resume.json'), 'w'), indent=2)
