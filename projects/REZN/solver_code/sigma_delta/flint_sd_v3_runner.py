"""Runner for V3 flint sigma-delta at any G_FULL and either clearing mode.

Usage: python flint_sd_v3_runner.py <crra|cara> <G_FULL> <dps> <max_iter> [IC]
  e.g.   python flint_sd_v3_runner.py cara 17 50 40 FR
         python flint_sd_v3_runner.py crra 17 50 40 NL
"""
...
from flint_sd_v3 import (mp, phi_sigdelta_v3, set_boundary, f_inf, to_np)
...
tag = f"v3_{CLEARING}_G{G_INNER}_dps{DPS}_{IC}"
...
Wd = 0.5*F0 + 0.5*F1; Wd = Wd / max(Wd.sum(), 1e-30)
def d_FR_weighted(P_inner):
    return float(np.sqrt(np.sum((P_inner - P_FR_in)**2 * Wd)))
def d_FR_unweighted(P_inner):
    return float(np.sqrt(np.mean((P_inner - P_FR_in)**2)))
def weighted_R2(P_inner):
    eps = 1e-30; Pc = np.clip(P_inner, eps, 1-eps); lp = np.log(Pc/(1-Pc))
...
if IC == 'FR':
    ...
    def crra_clear_f64(mu0, mu1, mu2, gf, steps=80):
        ...
        for _ in range(steps):
            m = (a+b)/2; e = dd(mu0,m)+dd(mu1,m)+dd(mu2,m)
            if e>0: a=m
            else: b=m
        return (a+b)/2
    ...
    for i in range(G_INNER):
        for j in range(G_INNER):
            for k in range(G_INNER):
                P_inner_ic[i,j,k] = crra_clear_f64(mu1[i,j,k], mu2[i,j,k], mu3[i,j,k], GAMMA_F)
...
lg(f"flint sigma-delta V3 -- {CLEARING.upper()} G_FULL={G_FULL} (G_inner={G_INNER}), dps={DPS}, IC={IC}, damped Picard.")
lg("Duplicate-crossing fix: bug found and patched.")

omega = mp(1); res_prev = mp('1e100')
hist = {'res': [], 'omega': [], 'd_FR_weighted': [], 'd_FR_unweighted': [],
        'slope': [], 'one_minus_R2': [], 'sec_per_iter': []}
t0 = time.time()
for it in range(1, MAX_ITER+1):
    t_step = time.time()
    P_phi = phi_sigdelta_v3(P, xi_u1, xi_S, xi_d, TOT_u_mp, TOT_S_mp, TOT_d_mp, tau, gamma, W,
    ...
    Pnp = to_np(P)
    inner_now = Pnp[INNER_LO:INNER_HI, INNER_LO:INNER_HI, INNER_LO:INNER_HI]
    d_w = d_FR_weighted(inner_now); d_uw = d_FR_unweighted(inner_now)
    omr2, slope, intc = weighted_R2(inner_now)
    sec = time.time() - t_step
    hist['res'].append(res); hist['omega'].append(float(omega))
    hist['d_FR_weighted'].append(d_w); hist['d_FR_unweighted'].append(d_uw)
    hist['slope'].append(slope); hist['one_minus_R2'].append(omr2)
    hist['sec_per_iter'].append(sec)
    ...
    json.dump({**hist, 'G_FULL': G_FULL, 'dps': DPS, 'tau': TAU_F, 'gamma': GAMMA_F,
               'IC': IC, 'clearing': CLEARING},
              open(os.path.join(HERE, f'flint_{tag}.json'), 'w'), indent=2)
    np.save(os.path.join(HERE, f'flint_{tag}_P.npy'), inner_now)
