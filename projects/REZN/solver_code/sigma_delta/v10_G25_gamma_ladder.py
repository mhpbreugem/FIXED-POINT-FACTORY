"""V10 (numba h-free smooth GL co-area) gamma-ladder at G_inner=25, STRICT TOL.
...
LOG = os.path.join(HERE, 'v10_G25_ladder.log')
open(LOG, 'w').close()
def lg(m):
...
for gv in GAMMAS:
    ...
    results.append(rec)
    np.save(os.path.join(HERE, f'v10_G25_gamma{gv:g}_P.npy'), inner)
    json.dump({'tau':TAU, 'G_inner':G_INNER, 'TOL':TOL, 'results':results,
               'note':'V10 h-free smooth GL strict-tol gamma ladder'},
              open(os.path.join(HERE, 'v10_G25_ladder.json'), 'w'), indent=2)
    if converged:
