"""V5 KERNEL CO-AREA low-gamma sweep at G_inner=10. Extends the main sweep
with very deep-CRRA values gamma in {0.01, 0.02, 0.05}.

At low gamma, the Jensen gap should be largest -- deeper Picard convergence
expected. CRRA bisection works but slower (R = exp((lm-lp)/gamma) can be
extreme). Bumped bisection steps to 300 for safety.
"""
...
LOG = os.path.join(HERE, 'flint_v5_gamma_low.log')
open(LOG, 'w').close()
def lg(m):
...
for gamma_val in GAMMAS:
    lg(f'=== LOW GAMMA = {gamma_val} ===  (h={float(h_kernel):.3f})')
    ...
    json.dump({'tau':TAU_F, 'G_FULL':G_FULL, 'dps':DPS, 'h':float(h_kernel), 'results':results,
               'note':'V5 kernel co-area LOW gamma extension'},
              open(os.path.join(HERE, 'flint_v5_gamma_low.json'), 'w'), indent=2)
