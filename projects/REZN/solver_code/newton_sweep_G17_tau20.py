#!/usr/bin/env python3
"""Newton JFNK continuation — adaptive step + quadratic extrapolation predictor.
Anchor: g050_t0200_G17.npz  (gamma=0.5, tau=2.0, G_inner=17, umax=4)
Sweeps left (gamma decreasing from 0.5 toward 0.01).
"""
...
# ── Anchor ────────────────────────────────────────────────────────────────────
ANCHOR = CKPT / "g050_t0200_G17.npz"
...
log(f"Anchor {ANCHOR.name}: G_inner={G_inner} pad={pad} gamma={anchor_gamma} "
    f"tau={tau_fixed} kernel_h={kernel_h:.4f} inner u=[{u_full[lo]:.2f},{u_full[hi-1]:.2f}]")
...
anchor_F = float(np.max(np.abs((phi_factory(anchor_gamma)(P_anchor) - P_anchor)[sl, sl, sl])))
results.append({"gamma": anchor_gamma, "one_minus_R2": float(r0),
                "F_reinsert": anchor_F, "newton_iters": 0, "step": 0.0})
log(f"  anchor  gamma={anchor_gamma:.5f}  1-R2={r0:.6e}  ||F||={anchor_F:.2e}")
...
meta = {"generated_at": datetime.now().isoformat(),
        "phase": "adaptive Newton JFNK, quadratic predictor, G17 tau=2.0 from gamma=0.5",
        "anchor_file": ANCHOR.name,
        "operator": f"phi_K3_halo_smooth kernel_h={kernel_h}",
        "tau": tau_fixed, "rows": results}
out_path = OUT / "newton_sweep_G17_tau20.json"
