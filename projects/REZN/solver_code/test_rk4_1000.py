#!/usr/bin/env python3
"""RK4 ODE sweep — 50-point gamma grid [0.1, 1000], f64-only."""
...
# ── Run RK4 sweep ────────────────────────────────────────────────────────────
log("=== RK4 sweep: predictor + corrector, 50-point grid [0.1, 1000] ===")
mp.dps = 50
t0 = time.time()
sweep = solve_sweep_rk4(
    ...
    gmres_restart     = 30,
    gmres_maxiter     = 5,
    f64_tol           = 1e-12,    # corrector target — keeps ‖φ(P)−P‖ bounded
    corrector_max_iter= 250,      # RK4 predictor + Anderson corrector each step
    anderson_m        = 5,
    mp_max_iter       = 0,
    verbose           = True,
)
...
for idx, (g, P_full) in enumerate(zip(sweep["gamma_grid"], sweep["P_outputs"])):
    ...
    try:
        ...
        rows.append({"gamma": float(g), "one_minus_R2": float(r2), "F_f64": F_f64})
        log(f"  gamma={g:.4f}  1-R2={r2:.5f}  F_f64={F_f64:.2e}")
