#!/usr/bin/env python3
"""RK4 ODE sweep — 13-point gamma grid [0.1, 10], f64-only."""
...
# ── 13-point gamma grid [0.1, 10] ───────────────────────────────────────────
gamma_grid = [float(10**x) for x in np.linspace(np.log10(0.10), np.log10(10.0), 13)]
...
# ── Run RK4 sweep ────────────────────────────────────────────────────────────
log("=== RK4 sweep: f64-only, 13-point grid [0.1, 10] ===")
mp.dps = 50
t0 = time.time()
sweep = solve_sweep_rk4(
    ...
    anderson_m        = 5,
    mp_max_iter       = 0,       # f64 is sufficient
    verbose           = True,
)
